...
impl Task for PythonRequestTask {
  ...
  fn compute(&mut self) -> Result<Self::Output> {
    ...
    let request = self.request.take().ok_or_else(|| {
      Error::from_reason("Request already consumed")
    })?;

    // Use the shared fallback runtime handle
    asgi::fallback_handle().block_on(async {
      // Send pending body data if present (from Request constructor)
      if let Some(body_buffer) = request.extensions().get::<BodyBuffer>() {
      ...
      // Always send End marker to close the request stream
      request
        .body()
        .end()
        .await
      ...
      // SAME handler call as handleStream - returns when headers are ready
      let response = self
        .asgi
        .handle(request)
        .await
      ...
      Ok::<http::Response<ResponseBody<Bytes>>, Error>(buffered_response)
    })
  }
...
impl Task for PythonStreamTask {
  ...
  fn compute(&mut self) -> Result<Self::Output> {
    ...
    asgi::fallback_handle().block_on(async {
      ...
      let is_websocket = request.extensions().get::<http_handler::WebSocketMode>().is_some();
      ...
      // Send End marker to close the request stream, but only for non-WebSocket requests
      // WebSocket connections manage their own lifecycle through websocket.receive messages
      if !is_websocket {
        request
          .body()
          .end()
      ...
      // Pass ownership of the request to the handler
      self.asgi
        .handle(request)
        .await
        .map_err(|e| Error::from_reason(e.to_string()))
